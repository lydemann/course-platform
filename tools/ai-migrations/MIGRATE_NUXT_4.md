...
const { data } = await useAsyncData(async () => $fetch(`/api/page/${route.params.slug}`));

// After (safe - key includes slug)
const { data } = await useAsyncData(route.params.slug, async () => $fetch(`/api/page/${route.params.slug}`));
```
