...
describe('Utils', () => {
  ...
  describe('mergeStates', () => {
    ...
    it('should overwrite the state', () => {
      expect(mergeStates(initial, transfer, MergeStrategy.OVERWRITE)).toEqual({
        1: 2,
      });
    });

    it('should merge over the state', () => {
      expect(mergeStates(initial, transfer, MergeStrategy.MERGE_OVER)).toEqual({
        1: 2,
        2: 2,
      });
    });
    ...
    it('should merge under the state', () => {
      expect(mergeStates(initial, transfer, MergeStrategy.MERGE_UNDER)).toEqual(
        { 1: 1, 2: 2 }
      );
    });
  });
